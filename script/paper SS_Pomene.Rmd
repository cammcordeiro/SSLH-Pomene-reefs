...
source("~/github/script/relatorio data wrangling.R")
...
dados1 %>% 
  mutate(uniqueID = paste(local, site, transecto, sep = "_")) %>% 
  aggregate(spp.Name ~local, ., function(x) length(unique(x))) 
...
dados1 %>% 
  ...
  aggregate(spp.Name ~ uniqueID + local, ., function(x) length(unique(x))) %>%
    mutate(local = mapvalues(local, from = "Zambia", to = "Baixo Zambia")) %>% 
  aov(spp.Name ~ local, .) %>% TukeyHSD("local")
  #glm(taxon ~ locality, ., family = 'poisson') %>% car::Anova()
  
# Evenness benthos ANOVA
data.frame(wideb[,1:3], eveness=diversity(wideb[,4:81])/log(specnumber(wideb[,4:81]))) %>%
  aov(eveness ~ locality, .) %>% TukeyHSD()
  
# ANOVA benthos Simpson
data.frame(wideb[,1:3], simpson=diversity(wideb[,4:81], index='simpson')) %>%
  aov(simpson ~ locality, .) %>% TukeyHSD("locality")
...
## benthic cover
#
betadisper(vegdist(wideb[, 4:81], method = "euclidian"), group = wideb$locality) %>% anova() #TukeyHSD()
...
## megafauna 
#
betadisper(vegdist(wide_macro[, 4:29], method = "bray"), group = wide_macro$locality) %>% anova() #TukeyHSD()
...
adonis(widef_sel[, 4:39] ~ locality, widef_sel, permutations = 4999, strata = widef_sel$site) 
