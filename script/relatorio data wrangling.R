...
traits1 <-read.xlsx("~/Google Drive/LECAR/data/Traits_Gaspar.xlsx", sheet=1)
head(traits1)
traits <- traits1[-1,]
head(traits)
...
wideb <- dados1 %>% 
  mutate(unique_eventID = paste(local, site, transecto, sep = "_")) %>% 
  aggregate(Cov..per.species ~ spp.Name + MFG + site + local + unique_eventID, ., mean) %>% 
  ...
  dcast(unique_eventID + site + local ~ spp.Name, value.var="Cov..per.species", sum) %>%
